...
async def create_user(user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    result = await db.execute(select(models.User).options(selectinload(models.Post.author)).where(models.User.email == user.email))
...
async def get_user_posts(user_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    result = await db.execute(select(models.Post).options(selectinload(models.Post.author)).where(models.User.id == user_id))
    posts = result.scalars().all()
    return posts
...
async def update_post_partial(post_id: int, post_data: PostUpdate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    update_data = post_data.model_dump(exclude_unset=True) # exclude_unset skips data with None
